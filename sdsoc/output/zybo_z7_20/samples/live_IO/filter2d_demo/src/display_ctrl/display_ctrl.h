...
#include <stdlib.h>
#include <stdint.h>
#include "display_ctrl/drm_modes.h"
...
typedef struct {
		const char * vdmaUioFile; /*dynclk uio file descriptor*/
		const char * vtcUioFile; /*dynclk uio file descriptor*/
		const char * clkUioFile; /*dynclk uio file descriptor*/
		drm_mode_modeinfo vMode; /*Current Video mode*/
		uint8_t *framePtr[DISPLAY_NUM_FRAMES]; /* Array of pointers to the framebuffers */
		uint32_t stride; /* The line stride of the framebuffers, in bytes */
		double pxlFreq; /* Frequency of clock currently being generated */
		uint32_t curFrame; /* Current frame being displayed */
